            ...
            <div class="bg-white shadow overflow-hidden sm:rounded-lg border border-gray-200">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full w-[1400px] max-w-none divide-y divide-gray-200" style="overflow-y: hidden;">
                        <style>
                            ...
                            .sticky-col-2 {
                                position: sticky;
                                left: 56px; /* ancho de la primera columna (#) px-6 = 24px padding + 8px contenido aprox */
                                background: #fff;
                                z-index: 10;
                                box-shadow: 2px 0 6px -2px rgba(0,0,0,0.04);
                            }
                        ...
                        <thead class="bg-gradient-to-r from-[#002c5f] to-[#004a9f] text-white">
                            <tr>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider sticky-col">#</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider sticky-col-2">Correlativo</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Asunto</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Solicitado por</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Autorizado por</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Fecha Generación</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Última Modificación</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Documento</th>
                                <th scope="col" class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">Estado</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($referencias as $index => $ref)
                                <tr class="hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 sticky-col">{{ $index + 1 }}</td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $ref->solicitado_por ?? '---' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $ref->autorizado_por ?? '---' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $ref->created_at->format('d/m/Y H:i') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{{ $ref->updated_at->diffForHumans(null, false, 'es') }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                                        @if ($ref->documento)
                                            <a href="{{ asset('storage/' . $ref->documento) }}" target="_blank"
                                               ...
                                               title="Ver documento">
                                                <svg class="h-5 w-5 mr-1 group-hover:animate-bounce" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if ($ref->estado === 'pendiente')
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800 border border-yellow-200 flex items-center">
                                        ...
                                        @else
                                            <span class="px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800 border border-green-200 flex items-center">
                                                <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <div class="flex justify-center space-x-3">
                                            <a href="{{ route('referencias.show', $ref->id) }}"
                                               ...
                                               title="Ver detalle">
                                                ...
                                                <span class="absolute -bottom-6 left-1/2 transform -translate-x-1/2 bg-gray-900 text-white text-xs rounded px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                                    Ver
                                                </span>
                                            </a>
                ...
                <div class="bg-white px-4 py-3 flex items-center justify-between border-t border-gray-200 sm:px-6">
                    <div class="flex-1 flex justify-between sm:hidden">
                        ...
                        @else
                            <a href="{{ $referencias->previousPageUrl() }}" class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Anterior
                            </a>
                        @endif
                        
                        @if($referencias->hasMorePages())
                            <a href="{{ $referencias->nextPageUrl() }}" class="ml-3 relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50">
                                Siguiente
                            </a>
                        @else
